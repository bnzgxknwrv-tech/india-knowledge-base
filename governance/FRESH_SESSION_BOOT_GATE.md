...
**SLIMMED 2026-09-14, then re-hardened the same day.** `governance/BOOT_MANIFEST_V8.json`'s `central_required` was first cut to 7 files, then found too aggressive (a confirmed `READ_COMPLETE != MARK_WORKING_MODEL_ACTIVE` failure — see `governance/HOW_TO_WORK_WITH_MARK.md`'s history) and restored to **15 files**: this gate, `governance/INDIA_MASTER_BOOT.md`, `governance/CURRENT_TRUTH.md`, `governance/CURRENT_FRONTIER.md`, `governance/HOW_TO_WORK_WITH_MARK.md` (hard-locked, do not re-slim without Mark's explicit approval), `governance/GUARDRAILS.md`, `governance/MARK_TRAVEL_PREFERENCES_CURRENT.md`, `governance/INDIA_BEHAVIORAL_EXECUTION_CONTRACT.md`, `governance/INDIA_ACTIVE_MEMORY_COMPILATION_GATE.md`, `governance/MARK_LOCATION_NAMING_CONTEXT_PROTOCOL.md`, `governance/INDIA_HUMAN_CENTERED_COMPLEX_TRIP_PLANNING_STANDARD.md`, `governance/MARK_PERSON_PROVENANCE_PLACE_MEANING_RULE.md`, `governance/MARK_FACING_PLACE_CARD_TRAVEL_VALUE_RULE_2026-09-13.md`, `governance/MARK_DAY_BLOCK_ONLY_PDF_RULE_2026-09-13.md`, `governance/MARK_TO_INDIA_SUCCESSOR_HUMAN_HANDOFF.md`. Read those fifteen, then the receipt/CHECK/authorization mechanics below still apply exactly as written, **plus** the mandatory Mark-working-model active-recall startcheck in `HOW_TO_WORK_WITH_MARK.md` §21 before content authorization. This is still far short of the old 19+14-file set (the rest of which stays on disk as on-demand archive; see `governance/ARCHIVE_INDEX.md`) — the difference is that this time the cut was checked against Mark's actual working model, not just token count.
...
## WHY BYTE-READ ALONE IS NOT ENOUGH
A new failure class was reproduced by INDIA15 on 2026-09-02: a session can read/prove all mandatory bytes yet still act from an unreconciled mixture of newer and stale sources. Examples found in real mandatory/current routing included:
- `CURRENT_STATE.md` corrected the frontier while `CURRENT_DECISIONS_MASTER.md`, `INDIA_CURRENT_KNOWLEDGE_MAP.md` and the manifest still pointed to optional-world survival;
- cluster file indexes labeled later traveler repairs non-blocking even though Mark had never seen the repaired findings;
- a mandatory presentation standard had been read but was not applied in the answer;
- A/B/C letters reserved for Mark grades were reused as bundle labels.

Therefore a valid boot requires both:
`READ ALL REQUIRED SOURCES` **and** `COMPILE/RECONCILE THEM INTO ONE CURRENT WORKING MODEL`.
...
4. read every file in manifest `central_required` completely at the pinned snapshot;
5. read every file in manifest `cci_required` completely at the manifest's immutable `cci_commit`;
6. read every file in manifest `active_cluster_required` completely;
...
9. **POST-READ COMPILATION / CONTRADICTION PASS:** compare at minimum `CURRENT_STATE.md`, `SUCCESSOR_SAFE_STATE.md`, `CURRENT_DECISIONS_MASTER.md`, `INDIA_CURRENT_KNOWLEDGE_MAP.md`, the manifest `active_cluster`, active-cluster package and relevant newer Mark decision/correction files. Determine one current answer for:
   - authority precedence;
   - exact current frontier;
   - exact first substantive action;
   - items/grades/locks that must not be re-presented;
...
## MANIFEST COUNTS — NEVER HARDCODE HERE
All mandatory counts and path sets come ONLY from the current `governance/BOOT_MANIFEST_V8.json` at the pinned boot snapshot.

Do not maintain a copied `16/6/6`, `16/6/10` or any other count here. The active-cluster set is expected to change as the project moves. If prose elsewhere contains a stale count, the manifest wins and the stale prose must be repaired.
