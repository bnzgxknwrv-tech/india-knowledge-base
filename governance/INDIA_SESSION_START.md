# INDIA_SESSION_START — HARD COMPATIBILITY POINTER
...
- no optional-cluster ballot before the six fixed cores are duration-closed and their mandatory transfers are budgeted;
...
Before every reply after a material event ask:
`CAN INDIA(N+1) CONTINUE FROM GITHUB WITHOUT MARK REPEATING ANYTHING?`

If NO: checkpoint first.

A durable checkpoint must include, where relevant:
...
- exact files the successor must read.
