...
17. `governance/INDIA_SESSION_TRANSITION_PROTOCOL.md` — **HARD EXTRACTION-FIRST HANDOFF PROTOCOL**: before any INDIA<N> session is treated as gone, its own live memory must be dumped to GitHub first — git history alone can never recover a conversation thread that was never written down. Read completely before participating in, or triggering, any session transition.
18. `governance/INDIA_POST_BOOT_ACTIVE_MEMORY_PARITY_CHECK.md` — **HARD UNIVERSAL SECOND-BOOT QUESTION.** Immediately after `CONTENT_AUTHORIZATION: GRANTED` and before any substantive reply, self-execute the FULL question in this file (or the LIGHT variant for an ordinary same-session continuation only). Proves the mandatory boot bytes were not merely read but actually compiled into applied working memory, including how to communicate with Mark, not only travel content.
19. `governance/INDIA14_START_AND_INDEPENDENT_CHECK.md` — **HARD START + INDEPENDENT CHECK PROTOCOL**: the canonical two/three-commit HEAD shape, the receipt/CHECK mechanics, §2.0's FULL-vs-LIGHT tier determination, the mandatory manifest-derived semantic veto topic set for FULL (`BOOT_MANIFEST_V8.json`'s `check_required_challenge_topics` — read the manifest for the current exact list/count, never a count hand-copied into prose), and the deterministic LIGHT spot-check procedure (§2.8); read completely, not merely invoked from memory.
20. `governance/EXACT_DELEGATION_BINDING_RULE.md` — **HARD CCI/WORK DELEGATION BINDING**: every delegation to CCI or WORK must bind the exact task comment, commit/head, artifact and expected result header; vague instructions like "voer de nieuwste taak uit" are forbidden whenever more than one task/version could exist.
21. `governance/MARK_TO_INDIA_SUCCESSOR_HUMAN_HANDOFF.md` — **HARD ACTIVE-MEMORY CHECKSUM OF REPEATED SUCCESSOR FAILURES.** This file is deliberately central-required so every material change to it changes the exact FULL-baseline blob set. It is ALSO the final `active_cluster_required` file and MUST be reread after CCI + active-cluster reconciliation as the last semantic boot read; its `LAATSTE PRE-ANSWER TEST — HARD VETO` is then applied before every substantive reply.

Boot is not complete because filenames were mentioned: actually read them. Historical copied count language is non-authoritative; the central mandatory read count is always `len(BOOT_MANIFEST_V8.json["central_required"])` — **21/21 at this repository state**, with the post-boot active-memory parity check added 2026-09-14. If this number and the manifest's own count ever disagree, the manifest wins and this prose must be repaired.
...
After the 21 central files above, every fresh INDIA successor MUST read the following CCI files from the immutable completed harvest commit `b5349afe41f98eb4870728aaff2c633899afc1fa` on `agent/cci-full-repo-knowledge-harvest`, in this order:

22. `runs/active/CCI-FULL-REPO-KNOWLEDGE-HARVEST-001/SUCCESSOR_START_HERE.md` — boot routing + parity warnings.
23. `runs/active/CCI-FULL-REPO-KNOWLEDGE-HARVEST-001/SUPERSEDED_AND_DO_NOT_REVIVE.md` — vocabulary drift, invalidated facts, old route/sleep modules and other anti-revival traps.
24. `runs/active/CCI-FULL-REPO-KNOWLEDGE-HARVEST-001/MARK_CURRENT_CANON_MASTER.md` — reconciled Mark decisions/WHY, including recovered items absent from the former central layer.
25. `runs/active/CCI-FULL-REPO-KNOWLEDGE-HARVEST-001/PROJECT_PHILOSOPHY_AND_SELECTION_MODEL.md` — recovered selection philosophy, AOAY/Top-11, NOT_TO_BE_MISSED, evidence and discovery rules needed to judge NEW findings correctly.
26. `runs/active/CCI-FULL-REPO-KNOWLEDGE-HARVEST-001/OPEN_MARK_DECISIONS_ONLY.md` — genuine open Mark gates plus INDIA-only repairs, conflicts and date-linked opportunities; ALWAYS reconcile against newer central state because the harvest froze against an older central commit.
27. `runs/active/CCI-FULL-REPO-KNOWLEDGE-HARVEST-001/CURRENT_TRAVEL_EXECUTION_CANON.md` — execution/transfer/topology/day-order facts, weak pins and live-recheck boundaries.
