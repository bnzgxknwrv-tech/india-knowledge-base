...
| **Kasar Devi Temple** (the Devi shrine itself, distinct from the cave) | **None found** — the Vivekananda meditation link belongs specifically to the cave entity above, not documented as the temple itself; no separate Yogananda/Sri Yukteswar/Ram Dass/NKB/Babaji/Anandamayi Ma tie found in archive | not independently graded | **NO on its own merit** — secondary/bycatch only, visitable at zero extra cost while already at the cave/ridge, never an independent reason to route there |
| **Hippie Hill / Crank's Ridge** (the wider seeker landscape, distinct from any single site on it) | Indirect — hosts the graded/candidate sites above, but "atmosphere" alone is not a person/lineage anchor | not independently graded | **NO on generic atmosphere alone** — see section 4a below: Mark has since stated an explicit personal power-place/sacred-landscape preference that may independently justify this beyond the person-link test; flagged, not resolved by the person-link filter alone |
...
### 4a. POWER-PLACE / SACRED-LANDSCAPE PREFERENCE (MARK_DELTA, PR #23) — NOT YET IN THE CENTRAL CANON

After this analysis was first posted, Mark stated a durable preference not previously captured: Crank's Ridge/Hippie Hill draws him not only via person-provenance but also as a **spiritually-charged/power-place landscape**, comparable to his own experience of Machu Picchu and Uluru/Ayers Rock — explicitly kept distinct from uncritical geophysical claims, but real as experiential/travel value. Mark himself flagged this should wait for a safe governance batch before entering `governance/MARK_TRAVEL_PREFERENCES_CURRENT.md` (a `central_required` file, currently frozen by the in-flight re-pin) — recorded now in full in `decisions/MARK_POWER_PLACE_SACRED_LANDSCAPE_PREFERENCE_2026-09-14.md` so it is not chat-only, queued for that same safe batch.

**Consequence for this analysis:** "Hippie Hill / Crank's Ridge" as landscape/atmosphere may have **independent intrinsic Mark value beyond the person-link filter** — the heavy-A table above tested it only against person-lineage, which is now confirmed to be an incomplete test for this specific entity. This does not re-open Jageshwar (no comparable landscape-preference statement exists for it) and does not change any grade. It does mean the ridge-as-landscape should not be dismissed as "generic hippie atmosphere" the way the original heavy-A filter's wording might have suggested — Mark's own words define it as something closer to Machu Picchu/Uluru in kind, not a diluted historical footnote.
